  ...
  return (
    <>
      <nav
        ...
        } ${lastScrollY > 10 ? 'bg-white/80 backdrop-blur-md shadow-sm border-b border-white/20' : 'bg-transparent'}`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center">
            ...
            <div className="flex-shrink-0">
              <Link to="/" className="text-4xl font-extrabold tracking-tighter text-gray-900 flex items-center gap-1">
                SHOP.CO
              </Link>
            </div>
            ...
            <div className="hidden md:flex items-center space-x-6">
              <div onClick={() => handleProtectedClick("/cart")} className="relative text-gray-700 hover:text-black transition cursor-pointer group">
                ...
                {cart.length > 0 && currentUser && (
                  <span className="absolute -top-1.5 -right-1.5 bg-black text-white text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center border-2 border-white">
              ...
              {!currentUser ? (
                <Link to="/login" className="px-5 py-2.5 bg-black text-white rounded-full text-sm font-medium hover:bg-gray-800 transition shadow-lg hover:shadow-xl">
                  Login
                </Link>
              ) : (
                <>
                  <Link to="/profile" className="text-gray-700 hover:text-black transition font-medium">Profile</Link>
                  <button onClick={logout} className="text-gray-700 hover:text-red-600 transition font-medium">Logout</button>
                </>
              )}
            </div>
            ...
            <div className="md:hidden">
              <button onClick={toggleMenu} className="p-2 text-gray-900 focus:outline-none">
            ...
            <div className="px-4 pt-4 pb-6 space-y-2">
              ...
              {!currentUser ? (
                ...
                <>
                  ...
                  <button onClick={() => { toggleMenu(); logout(); }} className="block w-full text-left px-3 py-3 rounded-lg text-lg font-medium text-red-600 hover:bg-red-50">Logout</button>
                </>
              )}
            </div>
          </div>
