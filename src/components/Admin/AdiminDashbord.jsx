...
const ProductsIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>;
const SalesIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-yellow-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v.01M12 12v4m0 4v.01M5 4h14a2 2 0 012 2v12a2 2 0 01-2 2H5a2 2 0 01-2-2V6a2 2 0 012-2z" /></svg>;
const RevenueIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>;
...
const AdminDashboard = () => {
    ...
    useEffect(() => {
        ...
        const processChartData = (orders, products) => {
            ...
            const sortedRevenue = Object.entries(revenueByProduct).sort((a, b) => b[1] - a[1]).slice(0, 5);
            const sortedQuantity = Object.entries(topSellingProducts).sort((a, b) => b[1] - a[1]).slice(0, 5);
            const sortedCategories = Object.entries(revenueByCategory).sort((a, b) => b[1] - a[1]);
            
            setChartData({
                revenueByProduct: {
                    labels: sortedRevenue.map(item => item[0]),
                    datasets: [{ label: 'Revenue (₹)', data: sortedRevenue.map(item => item[1]), backgroundColor: 'rgba(75, 192, 192, 0.6)' }],
                },
                topSellingProducts: {
                    labels: sortedQuantity.map(item => item[0]),
                    datasets: [{ label: 'Quantity Sold', data: sortedQuantity.map(item => item[1]), backgroundColor: 'rgba(255, 99, 132, 0.6)' }],
                },
                orderStatusDist: {
                    labels: Object.keys(orderStatusDist),
                    datasets: [{ data: Object.values(orderStatusDist), backgroundColor: ['#4CAF50', '#F44336', '#3B82F6'] }],
                },
                revenueByCategory: {
                    labels: sortedCategories.map(cat => cat[0]),
                    datasets: [{ label: 'Revenue (₹)', data: sortedCategories.map(cat => cat[1]), backgroundColor: 'rgba(153, 102, 255, 0.6)' }]
                }
            });
        };
